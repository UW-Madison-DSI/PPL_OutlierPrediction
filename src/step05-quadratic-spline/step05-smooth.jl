...
"""
    Calculate the y value the model generates before noise or outliers are added. Each chunk has its own quadratic term and a y intercept and linear picked so each chunk connects and first derivatives are equal.
...
function yValCalc(xs::Vector{Float64}, Buffer_y::Float64, Slopes::Vector{Float64}, Buffer_SubSlope::Float64)
    ...
    SubSlope = cumsum(pushfirst!([SubChunkSize * Slopes[i] for i=1:NumChunks], Buffer_SubSlope))
     ...
     # Calculates the change of y from the previous chunk to the current x. We combine this with a set of y ofset values in the next step to get the true mu fed into the normal distribution.
...
"""
    Creates a random model where the data is broken into chunks and a quadratic curve is fit on the data with noise and some probability that  they are outliers. the quadratic curves are picked such that they form a smooth line.
